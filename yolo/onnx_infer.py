...
class YOLOv8:
    ...
    def postprocess(self, input_image, output):
        ...
        for i in indices:
            ...
            x1, y1, w, h = box
            cropped_img = input_image[y1:y1+h, x1:x1+w]
            cropped = {
                "img": cropped_img,
                "box": box,
                "score": score,
    ...
    def main(self):
        ...
        # print("Input shape:", type(input_shape[2]))
        ...
        drawed_img, _ = self.postprocess(self.img, outputs)  # output image
        return drawed_img

    def detect_box_img(self):
        session = ort.InferenceSession(self.onnx_model, providers=[
                                       "CUDAExecutionProvider", "CPUExecutionProvider"])
        ...
        # Run inference using the preprocessed image data
        outputs = session.run(None, {model_inputs[0].name: img_data})
        _, cropped = self.postprocess(self.img, outputs)  # output image
        return cropped
